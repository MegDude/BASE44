      ...
      {selectedTarget ? <>
        ...
        <aside className="dp-relationship-drawer is-open" role="dialog" aria-modal="true" aria-labelledby="relationship-drawer-title">
          ...
          <div className="dp-relationship-drawer__body">
            <span className="dp-relationship-drawer__eyebrow dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">{selectedTarget.segment} · {selectedTarget.relationshipStrength}</span>
            ...
            <section><span className="dp-relationship-drawer__eyebrow dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">Approval and operating routes</span><p className="dp-relationship-drawer__contacts-note">Full contact detail from the working data. Reconfirm every route before outreach.</p><div className="dp-relationship-drawer__contacts">{(selectedTarget.contacts || []).map((contact, index) => { const href = contactLink(contact); const value = contactValue(contact) || "No direct email or phone supplied"; return <div key={`${contact.name}-${index}`}><strong>{contact.name}</strong><span>{contact.role}</span>{href ? <a href={href}>{value}</a> : <span>{value}</span>}<small>{contact.status}</small></div>; })}</div></section>
            <p className="dp-relationship-drawer__note"><strong>Immediate next action</strong><br />{selectedTarget.nextAction}</p>
            <div className="dp-relationship-drawer__copy"><span className="dp-relationship-drawer__eyebrow dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">Copy-ready approval ask</span><p>{selectedTarget.ask}</p><div><button type="button" onClick={() => copyText(`${selectedTarget.id}-ask`, selectedTarget.ask)}>{copied === `${selectedTarget.id}-ask` ? "Message copied" : "Copy message"}</button><button type="button" onClick={() => copyText(`${selectedTarget.id}-brief`, buildTargetBrief(selectedTarget))}>{copied === `${selectedTarget.id}-brief` ? "Brief copied" : "Copy full brief"}</button></div></div>
