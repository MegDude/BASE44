...
export function GlobalWorkspaceSearch({ open, onClose, scope }) {
  ...
  return createPortal(<div className="dp-workspace-search-layer" role="dialog" aria-modal="true" aria-label="Search workspace">
    ...
    <section className="dp-workspace-search-sheet">
      <header>
        <button type="button" onClick={onClose} aria-label="Go back from workspace search"><ArrowLeft aria-hidden="true" /></button>
        <Search aria-hidden="true" />
        <input autoFocus type="search" placeholder="Search workspace" aria-label="Search workspace" onInput={(event) => {
