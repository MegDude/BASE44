...
function ExperienceBuilder({ template, organizationId }) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      setPublishMessage(result.idempotentReplay
        ? "Already published. The existing experience is ready."
        : "Published. The experience is ready for its configured placements.");
    } catch (error) {
      setPublishState("error");
      setPublishMessage(error instanceof Error
        ? error.message
        : "Publishing is unavailable. Your draft remains open.");
    }
  };
