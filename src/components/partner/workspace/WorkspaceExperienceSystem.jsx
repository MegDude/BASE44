...
export function WorkspaceExperienceSystem({ organizationId, view = "campaigns" }) {
  ...
  return <motion.section className="dp-workspace-experiences" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.24 }} aria-labelledby="workspace-experiences-title">
    <header className="dp-workspace-experiences__header"><p>{surveyOnly ? "Surveys" : "Experiences"}</p><h1 id="workspace-experiences-title">{surveyOnly ? "Ask one useful question." : "Create one connected experience."}</h1><span>{surveyOnly ? "Questions can appear on the map, after an action, along a route, or through a shared link without requiring QR." : "Connect content, audience, placement, interaction, and one measurable result."}</span><button type="button" disabled={!defaultTemplate} onPointerUp={(event) => { event.preventDefault(); startDefaultTemplate(); }} onClick={(event) => { if (event.detail === 0) startDefaultTemplate(); }}>{surveyOnly ? "Create survey" : "Create experience"}</button></header>
    <section className="dp-workspace-experiences__flow" aria-labelledby="experience-flow-title"><h2 id="experience-flow-title">One system</h2><ol>{["Goal", "Experience", "Placement", "Interaction", "Result"].map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span><strong>{item}</strong></li>)}</ol></section>
