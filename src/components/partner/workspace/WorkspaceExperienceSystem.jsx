...
function ExperienceBuilder({ template, organizationId }) {
  ...
  if (step === 4) content = <BuilderChoiceList legend="Where should it appear?" options={PLACEMENTS} selected={draft.placements} onToggle={(value) => update({ placements: toggleValue(draft.placements, value) })} />;
...
export function WorkspaceExperienceSystem({ organizationId, view = "campaigns" }) {
  ...
  const start = (template) => openSheet({ eyebrow: surveyOnly ? "Survey" : "Campaign", title: template.label, state: "full", content: <ExperienceBuilder template={template} organizationId={organizationId} /> });
  const defaultTemplate = templates[0];

  return <motion.section className="dp-workspace-experiences" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.24 }} aria-labelledby="workspace-experiences-title">
    <header className="dp-workspace-experiences__header"><p>{surveyOnly ? "Surveys" : "Campaigns"}</p><h1 id="workspace-experiences-title">{surveyOnly ? "Ask one useful question." : "Create one connected experience."}</h1><span>{surveyOnly ? "Questions can appear on the map, after an action, along a route, or through a shared link without requiring QR." : "Choose one goal, audience, placement, and result."}</span><button type="button" onClick={() => start(defaultTemplate)}>{surveyOnly ? "Create survey" : "Create campaign"}</button></header>
    <section className="dp-workspace-experiences__flow" aria-labelledby="experience-flow-title"><h2 id="experience-flow-title">Campaign flow</h2><ol>{["Goal", "Content", "Placement", "Action", "Result"].map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span><strong>{item}</strong></li>)}</ol></section>
    <section className="dp-workspace-experiences__library" aria-labelledby="experience-library-title"><header><div><p>Library</p><h2 id="experience-library-title">Start with a useful structure.</h2></div></header>{groups.length > 1 ? <div className="dp-workspace-experiences__tabs" role="tablist" aria-label="Experience groups">{groups.map((group) => <button key={group} type="button" role="tab" aria-selected={group === activeGroup} onClick={() => setActiveGroup(group)}>{group}</button>)}</div> : null}<div className="dp-workspace-experiences__rows">{visible.map((template) => <button type="button" key={template.id} onClick={() => start(template)}><span><strong>{template.label}</strong><small>{template.description}</small><em>{template.content.length} modules · {template.placements.length} placements</em></span><ChevronRight aria-hidden="true" /></button>)}</div></section>
    {surveyOnly ? <aside className="dp-workspace-experiences__integration"><div><h2>Response storage</h2><p>Responses save to Downtown Perks first. Google Sheets is an optional workspace sync and never blocks a resident submission.</p></div><Link to={withPartnerWorkspaceContext("/partner-workspace/sources?section=google-sheets", organizationId)}>Review Google Sheets</Link></aside> : null}
  </motion.section>;
}
