...
function PlacesView({ analytics, range, updateQuery, workspaceId }) {
  return <AnalyticsDeepView eyebrow="Places" title="Location performance." description="Only entities attached to the active workspace are ranked."><RankedPlaces rows={analytics.places} detailed workspaceId={workspaceId} /><div className="dp-analytics-inline-actions"><button type="button" onClick={() => updateQuery({ view: "geography" })}>Compare geography</button><Link to={withPartnerWorkspaceContext(`/map?mode=partner&tab=map&range=${range}`, workspaceId)}>Open partner map</Link></div></AnalyticsDeepView>;
}

function CampaignsView({ analytics, workspaceId }) { return <AnalyticsDeepView eyebrow="Campaigns" title="Campaign comparison." description="Campaigns are compared only when workspace-scoped events contain stable campaign identifiers."><SimpleRows rows={analytics.campaigns.map((row) => [row.id, row.actions, "Recorded actions"])} empty="No campaign-linked activity was recorded for this period." workspaceId={workspaceId} /><DataNotice>Cost-per-action remains unavailable until campaign objective and spend records are connected.</DataNotice></AnalyticsDeepView>; }
...
function SourcesView({ analytics, workspaceId }) { return <AnalyticsDeepView eyebrow="Sources" title="Entry source attribution." description="This is first-touch source reporting from stable event identifiers, not multi-touch attribution."><SimpleRows rows={analytics.sources.map((row) => [row.label, row.actions, `${row.entries} entries · ${row.conversion === null ? "conversion unavailable" : `${row.conversion}% conversion`}`])} empty="No source identifiers were recorded." workspaceId={workspaceId} /></AnalyticsDeepView>; }
function GeographyView({ analytics, updateQuery, workspaceId }) { return <AnalyticsDeepView eyebrow="Geography" title="District and map performance." description="District selections deep-link to the canonical partner map."><div className="dp-analytics-map-layout"><div className="dp-analytics-map-bridge"><MapPin aria-hidden="true" /><h3>Canonical partner map</h3><p>Select an area to open the shared map layer and entity records.</p><Link to={withPartnerWorkspaceContext("/map?mode=partner&tab=map", workspaceId)}>Open map</Link></div><SimpleRows rows={analytics.geography.map((row) => [row.district, row.actions, "Recorded actions"])} empty="No district identifiers were recorded." onSelect={(district) => updateQuery({ district, view: "geography" })} workspaceId={workspaceId} /></div></AnalyticsDeepView>; }
function ReportsView({ workspaceId, range, comparison }) { const reports = [["Monthly performance", "/partner-workspace/reports"], ["Campaign report", "/partner-workspace/reports?type=campaign"], ["Place report", "/partner-workspace/reports?type=place"], ["Source attribution report", "/partner-workspace/reports?type=source"], ["Executive summary", "/partner-workspace/reports?type=executive"]]; return <AnalyticsDeepView eyebrow="Reports" title="Turn analysis into a documented read." description="Reports retain the active workspace and clearly state their period, definitions, limitations, and included records."><div className="dp-analytics-report-list">{reports.map(([label, href]) => <Link to={withPartnerWorkspaceContext(`${href}${href.includes("?") ? "&" : "?"}range=${range}&comparison=${comparison}`, workspaceId)} key={label}><strong>{label}</strong><span>Open report builder</span><ArrowRight aria-hidden="true" /></Link>)}</div></AnalyticsDeepView>; }
