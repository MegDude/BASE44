  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div className="dp-glass flex items-center gap-2 px-3 py-2.5 transition-all focus-within:border-[#BFA46A]/60 focus-within:bg-white/64 md:px-4 md:py-2.5">
          ...
          <button
            ...
            className={`p-2 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#BFA46A] ${
              isAI && !aiLoading
                ? 'bg-white/82 text-[#BFA46A] shadow-[0_0_0_1px_rgba(191,164,106,0.18),0_10px_24px_rgba(11,31,51,0.06)] hover:text-[#BFA46A] hover:shadow-[0_0_0_1px_rgba(191,164,106,0.28),0_12px_30px_rgba(11,31,51,0.08)]'
                : 'bg-white/58 text-[#BFA46A] opacity-80 shadow-[0_0_0_1px_rgba(191,164,106,0.10)]'
            }`}
            title="Ask the Map AI"
          >
            {aiLoading ? (
              <div className="w-4 h-4 border-2 border-white/30 border-t-[#BFA46A] animate-spin" />
            ) : (
        ...
        <AnimatePresence mode="wait" initial={false}>
          {isExpanded && !query && (
            <motion.div
              ...
              className="dp-glass-card absolute left-0 right-0 top-full z-[1400] mt-2 max-h-[min(420px,calc(100vh-150px))] overflow-y-auto bg-white/94 shadow-[0_24px_70px_rgba(11,31,51,0.16),0_0_0_1px_rgba(191,164,106,0.10)]"
            >
              {SEARCH_PROMPTS.map((prompt, i) => (
                ...
                >
                  <div className="flex items-start gap-2">
                    <Zap className="w-4 h-4 text-[#BFA46A] shrink-0 mt-0.5" />
                    <div>
