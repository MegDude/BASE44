...
function getNextState(state: DrawerSnapState): DrawerSnapState {
  if (state === "dismissed") return "medium";
  if (state === "full") return "medium";
  return states[Math.min(states.indexOf(state) + 1, states.length - 1)];
}
  ...
  return (
    <section className="dp-mobile-tab-drawer" data-drawer-state={state} role="dialog" aria-modal={state === "full"} aria-labelledby="dp-mobile-tab-title">
      <button
        ...
        aria-label={`Drawer size: ${state}. Activate to ${state === "full" ? "reduce" : "expand"}.`}
