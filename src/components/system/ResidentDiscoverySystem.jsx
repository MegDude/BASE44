          ...
          <button
            ...
            className={cn(
              ...
              active
                ...
                : "border-[#0B1F33]/8 bg-white text-[#0B1F33]/70 hover:border-[#BFA46A]/50 hover:text-[#0B1F33]"
            )}
          >
            {active && (
              <motion.span
            ...
            <span className="relative z-10 flex items-center justify-between gap-3">
              ...
              <Icon className={cn("h-3.5 w-3.5", active ? "text-[#BFA46A]" : "text-[#0B1F33]/42")} />
            </span>
            <span className="relative z-10 mt-3 block h-px overflow-hidden bg-white/10">
              <motion.span
                className="block h-full bg-[#BFA46A]"
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <Icon className={cn("h-5 w-5 drop-shadow-[0_8px_16px_rgba(11,31,51,0.12)]", active ? "text-[#BFA46A]" : "text-[#0B1F33]/62")} />
        {label && (
...
function MapPreviewPanel({ activeCategory }) {
  return (
    <div className="relative min-h-[430px] overflow-hidden rounded-[10px] border border-[#0B1F33]/8 bg-white">
      ...
      <motion.div
        className="absolute left-[28%] top-[43%] h-[2px] w-[42%] origin-left rotate-[-18deg] bg-[#BFA46A]"
        initial={{ scaleX: 0 }}
        animate={{ scaleX: 1 }}
      ...
      >
        <div className="mb-2 flex items-center gap-2 text-[#BFA46A] dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">
      ...
      <div className="absolute bottom-5 left-5 right-5">
        <div className="max-w-md rounded-[6px] border border-[#0B1F33]/8 bg-white/92 p-4 shadow-[0_14px_34px_rgba(11,31,51,0.05)]">
          <div className="mb-2 text-[#BFA46A] dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">5-minute walk</div>
  ...
  return (
    <div className="pt-2">
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          <span className="dp-label mb-2 block dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">{category.eyebrow}</span>
          ...
          <div className="mt-5 divide-y divide-[#0B1F33]/8 border-y border-[#0B1F33]/8">
            {category.cards.map((card, index) => (
              ...
              >
                ...
                <span className="text-[#0B1F33]/38 dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">
  ...
  return (
    ...
    >
      <SplitLayout className="items-start gap-5 md:grid-cols-[0.8fr_1.2fr]">
        <div>
          <span className="dp-label mb-2 block dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">How It Works</span>
          ...
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <span className="dp-label dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">{step.label}</span>
              ...
              <div className="mt-5">
                ...
                {stepIndex === 1 && (
                  <div className="relative h-24">
                    <motion.div className="absolute left-4 top-10 h-[2px] w-[210px] origin-left bg-[#BFA46A]" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8, ease }} />
                    <div className="absolute left-1 top-8 text-[#0B1F33]/68 drop-shadow-[0_10px_18px_rgba(11,31,51,0.12)]">
                      <MapPin className="h-5 w-5" />
                    </div>
                    <div className="absolute right-6 top-8 text-[#BFA46A] drop-shadow-[0_10px_18px_rgba(11,31,51,0.14)]">
                ...
                {stepIndex === 2 && (
                  <motion.div className="relative max-w-[260px] rounded-[10px] border border-[#0B1F33]/10 bg-[#0B1F33] p-4 text-white shadow-[0_18px_46px_rgba(11,31,51,0.18)]" initial={{ y: 22 }} animate={{ y: 0 }} transition={{ duration: 0.45, ease }}>
                    <div className="flex items-center justify-between">
                      <span className="text-white/58 dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">Resident Card</span>
                      <CreditCard className="h-4 w-4 text-[#BFA46A]" />
                    </div>
                    <div className="mt-8 font-heading text-xl font-medium">Perk confirmed</div>
                    <motion.div className="absolute left-3 right-3 top-1/2 h-px bg-[#BFA46A]" initial={{ x: "-110%" }} animate={{ x: "110%" }} transition={{ duration: 1.2, ease }} />
                  </motion.div>
                )}
              </div>
            </motion.div>
  ...
  return (
    <div className="border-t border-[#0B1F33]/8 pt-6">
      <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <span className="dp-label mb-2 block dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">What’s Around the Corner</span>
  ...
  return (
    <SectionContainer>
      <SplitLayout className="mb-8 items-end">
        <RevealSection>
          <span className="dp-label mb-4 block dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">Resident Experience</span>
