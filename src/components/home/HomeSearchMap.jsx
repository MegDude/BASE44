...
export default function HomeSearchMap({ searchCopy, mapCopy }) {
  ...
  function handleSubmit(nextQuery) {
    const resolvedQuery = String(nextQuery || query).trim();
    setSubmittedQuery(resolvedQuery);
    setShowFilters(false);
    navigate(`/explore${resolvedQuery ? `?q=${encodeURIComponent(resolvedQuery)}` : ""}`);
  }
        ...
        <div>
          ...
          <div className="relative mt-4">
                  ...
                  {searchCopy.filterChips.map((chip) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setShowFilters(false);
                        navigate(`/explore?filter=${encodeURIComponent(chip.label.toLowerCase())}`);
                      }}
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="rounded-full border border-[rgba(15,23,42,0.10)] bg-white px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[rgba(71,85,105,0.94)]">
                Filter: <span className="text-[var(--dp-navy,#111827)]">{activeFilter}</span>
              </div>
              <button
                type="button"
                onClick={() => setShowFilters((current) => !current)}
                className="inline-flex min-h-[42px] items-center rounded-full border border-[rgba(15,23,42,0.10)] bg-white px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[rgba(71,85,105,0.94)]"
              >
                {showFilters ? "Hide filters" : "Browse filters"}
              </button>
            </div>
          </div>
          <p className="mt-4 text-[13px] leading-6 text-[rgba(71,85,105,0.94)]">{searchCopy.summary}</p>
        </div>
