Warning: truncated output (original token count: 228620)
Total output lines: 19383
...
function isHotelEntity(place) {
  ...
  const partnerType = getExplicitPartnerType(place);
  const name = String(place?.name || place?.title || place?.raw?.name || place?.raw?.title || "").toLowerCase();
  return (…178620 tokens truncated…}&tab=map&filter=${encodeURIComponent(activeFilter || "All")}`, { replace: true });
