...
export default function ResidentWelcome() {
  ...
  if (!isLoadingAuth && !isAuthenticated) return <main className="dp-membership-page"><section className="dp-membership-join dp-resident-welcome-auth"><div><p className="dp-membership-eyebrow">Resident account</p><h1>Finish your membership.</h1><p>Use the same email you entered when you checked your building or completed payment. That keeps your membership and resident profile together.</p><div className="dp-membership-actions"><Link to={`/residents/login?mode=register&returnTo=${encodeURIComponent(returnTo)}`}>Create account</Link><Link to={`/residents/login?returnTo=${encodeURIComponent(returnTo)}`}>Sign in</Link></div></div></section></main>;

  return <main className="dp-membership-page"><section className="dp-resident-welcome" aria-labelledby="resident-welcome-title">
    <p className="dp-membership-eyebrow">Welcome to Downtown Perks</p><h1 id="resident-welcome-title">{membership && membership.source === "free_building" ? "Your building includes membership." : "Your resident membership is active."}</h1><p>{message}</p>
