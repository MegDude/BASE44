...
export default function ResidentWelcome() {
  ...
  if (!isLoadingAuth && !isAuthenticated) return <main className="dp-membership-page"><section className="dp-membership-join dp-resident-welcome-auth"><div><p className="dp-membership-eyebrow text-[11px] uppercase text-[11px] uppercase tracking-[0.15em] text-[11px] uppercase tracking-[0.15em] text-[11px] uppercase tracking-[0.15em] dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">Resident account</p><h1>Finish your membership.</h1><p>Use the same email you entered when you checked your building or completed payment. That keeps your membership and resident profile together.</p><div className="dp-membership-actions"><Link to={`/residents/login?mode=register&returnTo=${encodeURIComponent(returnTo)}`}>Create account</Link><Link to={`/residents/login?returnTo=${encodeURIComponent(returnTo)}`}>Sign in</Link></div></div></section></main>;

  return <main className="dp-membership-page"><section className="dp-resident-welcome" aria-labelledby="resident-welcome-title">
    <p className="dp-membership-eyebrow text-[11px] uppercase text-[11px] uppercase tracking-[0.15em] text-[11px] uppercase tracking-[0.15em] text-[11px] uppercase tracking-[0.15em] dp-eyebrow text-[11px] font-bold uppercase tracking-[0.15em]">Welcome to Downtown Perks</p><h1 id="resident-welcome-title">{membership && membership.source === "free_building" ? "Your building includes membership." : "Your resident membership is active."}</h1><p>{message}</p>
