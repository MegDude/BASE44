...
function buildingIcon(active) {
  return L.divIcon({
    className: "",
    html: `<div style="width:12px;height:12px;border-radius:3px;background:${active ? "#C8973A" : "#4a5568"};border:2px solid rgba(255,255,255,0.8);box-shadow:0 0 8px ${active ? "#C8973A80" : "#00000040"}"></div>`,
...
function nearbyIcon() {
  return L.divIcon({
    className: "",
    html: `<div style="width:8px;height:8px;border-radius:50%;background:#5B9E6E;border:2px solid rgba(255,255,255,0.7);box-shadow:0 0 5px #5B9E6E60"></div>`,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-14 items-start">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <a href="#property-map" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full border border-border/70 text-foreground/70 font-medium text-sm hover:text-foreground transition-all">
              ...
              {!building ? (
                ...
                <motion.div key={building.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                  <div className="p-5 border-b border-border/40 flex items-center justify-between">
                    <div>
                      <div className="font-medium text-foreground text-sm">{building.name}</div>
    ...
    <section ref={ref} className="py-10 px-6 border-t border-border/40">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="mb-10">
          ...
          <h2 className="font-heading text-2xl md:text-3xl font-medium tracking-tight">From building to live downtown layer.</h2>
        </motion.div>
          ...
          {buildings.slice(0, 3).map((b, i) => (
            <motion.div key={b.id} initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.08 }}
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="font-heading font-medium text-sm text-foreground">{b.name}</span>
              </div>
        ...
        <div className="border border-border/50 rounded-xl overflow-hidden">
          <div className="flex border-b border-border/40 overflow-x-auto">
            {TYPES.map(t => (
              <button key={t} onClick={() => setFormType(t)}
                className={`px-6 py-4 text-[12px] font-medium whitespace-nowrap border-r border-border/40 last:border-r-0 transition-all ${formType === t ? "text-primary bg-primary/5" : "text-muted-foreground hover:text-foreground"}`}>
                {t}
              </button>
            ))}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-8 md:border-r border-border/40 space-y-4">
              ...
              <button className="w-full py-3 rounded-full bg-primary text-primary-foreground font-medium text-sm hover:bg-primary/90 transition-all">Activate your building</button>
            </div>
              ...
              <div className="space-y-2 flex-1">
                {prompts.map(p => (
                  <button key={p} onClick={() => setFormText(p)}
                    className="w-full text-left px-4 py-3 rounded-lg border border-border/40 hover:border-primary/30 text-[13px] text-muted-foreground hover:text-foreground transition-all">{p}</button>
                ))}
              </div>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-3">
              <a href={ctaHref} className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-primary text-primary-foreground font-medium text-sm hover:bg-primary/90 transition-all">
                {ctaLabel} <ArrowRight className="w-4 h-4" />
              </a>
              <a href={secondHref} className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full border border-border/70 text-foreground/70 font-medium text-sm hover:text-foreground transition-all">
                {secondLabel}
              </a>
            </div>
