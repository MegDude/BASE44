...
const VENUE_FILTER_COPY = {
  ...
  walkable: {
    label: "Walkable now",
    title: "Show where the venue fits into nearby plans.",
    body: "Walkability connects a venue to buildings, hotels, events, and other places that shape the decision path.",
    result: "Logged-in view: directions, distance context, nearby routes, and district use.",
  },
  trending: {
    label: "Trending",
    ...
    result: "Logged-in view: searches, saves, views, campaign lift, and recommended next steps.",
  },
  saved: {
    label: "Saved nearby",
    title: "See what people save before a visit happens.",
    body: "Saves show which venues people plan to return to and which categories are building repeat interest.",
    result: "Logged-in view: saved users, categories people revisit, nearby places, and follow-up opportunities.",
  },
};
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-14 items-start">
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}>
              <span className="text-[11px] text-primary/70 uppercase block mb-4 text-[11px] font-bold uppercase tracking-normal">Venue Partner Layer</span>
      ...
      <section id="venue-map" className={`${PARTNER_SPACING.subsectionVertical} px-5 border-t border-border/40`}>
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 items-end">
            <div>
              <span className="text-[11px] text-primary/70 uppercase block mb-3 text-[11px] font-bold uppercase tracking-normal">Venue Map</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 overflow-hidden" style={{ height: 480 }}>
              <PartnerMapIntelligenceLayer
                ...
                insight="Venues, offers, walkable plans, and decision moments shown in context."
                kind="venue"
              ...
              {!venue ? (
                <>
                  <div className="p-5 border-b border-border/40">
                    <div className="text-[11px] text-muted-foreground uppercase mb-1 text-[11px] font-bold uppercase tracking-normal">Decision moments happening now</div>
                  </div>
                  ...
                  <div className="p-4 border-t border-border/40">
                    <div className="text-[11px] text-muted-foreground uppercase mb-3 text-[11px] font-bold uppercase tracking-normal">Venues</div>
                    <div className="space-y-2">
                      {VENUES.slice(0, 4).map(v => (
                        <button key={v.id} onClick={() => selectVenue(v)} className="w-full flex items-center gap-2.5 p-2.5 rounded-lg border border-border/40 hover:border-primary/30 transition-all text-left">
                          <div className="w-2 h-2 rounded-[12px] shrink-0" style={{ background: CAT_COLORS[v.cat] || "#0B1F33" }} />
                ...
                <motion.div key={venue.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                  ...
                  <div className="flex-1 p-5 space-y-4 overflow-y-auto">
                    ...
                    <div>
                      <div className="text-[11px] text-muted-foreground uppercase mb-2 text-[11px] font-bold uppercase tracking-normal">Live now</div>
                    ...
                    <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
                      <div className="text-[11px] text-primary uppercase mb-1 text-[11px] font-bold uppercase tracking-normal">Offer active</div>
                  ...
                  <div className="p-4 border-t border-border/40 flex gap-2">
                    <button className="flex-1 py-2.5 rounded-[12px] bg-primary text-primary-foreground text-[12px] font-medium hover:bg-primary/90 transition-all">Save</button>
                    <button className="flex-1 py-2.5 rounded-[12px] border border-border/60 text-foreground/70 text-[12px] font-medium hover:text-foreground transition-all">Directions</button>
                    <button className="px-4 py-2.5 rounded-[12px] border border-border/60 text-foreground/70 text-[12px] font-medium hover:text-foreground transition-all">Add offer</button>
                  </div>
                </motion.div>
              )}
            </div>
    ...
    <section ref={ref} className={`${PARTNER_SPACING.subsectionVertical} px-5 border-t border-border/40`}>
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="mb-8">
          <span className="text-[11px] text-primary/70 uppercase block mb-3 text-[11px] font-bold uppercase tracking-normal">How people find places</span>
...
function VenueCards({ venues, selectVenue }) {
  ...
  return (
    <section ref={ref} className={`${PARTNER_SPACING.subsectionVertical} px-5 border-t border-border/40`}>
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="mb-8">
          <span className="text-[11px] text-primary/70 uppercase block mb-3 text-[11px] font-bold uppercase tracking-normal">Live venues</span>
          ...
          {venues.slice(0, 3).map((v, i) => (
            <motion.div key={v.id} initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.08 }}
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="w-2.5 h-2.5 rounded-[12px] shrink-0" style={{ background: CAT_COLORS[v.cat] || "#0B1F33" }} />
              ...
              <div className="h-1.5 rounded-[12px] bg-border/50 mb-4 overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={inView ? { width: `${Math.min(100, (v.views / 130) * 100)}%` } : {}} transition={{ duration: 1, delay: 0.3 + i * 0.1 }} className="h-full rounded-[12px] bg-primary" />
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-border/40">
                <div className="text-[10px] text-muted-foreground/60 uppercase mb-1 text-[11px] font-bold uppercase tracking-normal">Top trigger</div>
        ...
        <div className="border border-border/50 overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-8 bg-muted/10 flex flex-col">
              <p className="text-[13px] leading-6 text-[#0B1F33]/68">Share the place, the offer or event, and the nearby moment you want to show up for.</p>
            </div>
          </div>
        </div>
      </div>
