...
export default function PartnerRolePage() {
  ...
  const campaignType = campaignTypeByRole[role] || "venues";
  return (
      ...
      <section className="mx-auto grid max-w-5xl gap-8 rounded-[18px] border border-[rgba(7,27,47,0.08)] bg-white p-6 md:grid-cols-[1.2fr_.8fr] md:p-10">
        <div>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link to={`/partners/campaigns?type=${campaignType}#launch-campaign`} className="inline-flex h-11 items-center justify-center rounded-[10px] bg-[#071B2F] px-5 text-[12px] font-bold uppercase tracking-normal text-white">{config.cta}</Link>
            <Link to="/map?mode=partner&tab=campaigns" className="inline-flex h-11 items-center justify-center rounded-[10px] border border-[rgba(7,27,47,0.18)] bg-white px-5 text-[12px] font-bold uppercase tracking-normal text-[#071B2F]">Open map</Link>
          </div>
        </div>
