...
export async function loadWorkspaceAnalytics({ workspace, entities = [], range = "30d", comparison = "previous_period", from = null, to = null, filters = {} }) {
  ...
  const period = getPeriodWindow({ range, comparison, from, to });
  ...
  const previousEvents = comparison === "none" ? [] : allEvents.filter((event) => isInWindow(event.occurredAt, period.comparisonFrom, period.comparisonTo));
  if (events.length === 0) {
    return {
...
export function getPeriodWindow({ range = "30d", comparison = "previous_period", from = null, to = null } = {}) {
  ...
  const start = range === "ytd"
    ...
    : customStart || new Date(end.getTime() - (days - 1) * 24 * 60 * 60 * 1000);
  const duration = Math.max(1, end.getTime() - start.getTime() + 1);
  const comparisonTo = comparison === "previous_year"
    ? new Date(end.getFullYear() - 1, end.getMonth(), end.getDate(), end.getHours(), end.getMinutes(), end.getSeconds(), end.getMilliseconds())
    : comparison === "none" ? end : new Date(start.getTime() - 1);
  const comparisonFrom = comparison === "previous_year"
    ? new Date(start.getFullYear() - 1, start.getMonth(), start.getDate(), start.getHours(), start.getMinutes(), start.getSeconds(), start.getMilliseconds())
    : comparison === "none" ? start : new Date(comparisonTo.getTime() - duration + 1);
...
function isInRange(value, start) { const date = new Date(value || 0); return !Number.isNaN(date.getTime()) && date >= start; }
...
function dedupeEvents(events) {
  ...
  return events.filter((event, index) => {
    ...
    if (/save/i.test(event.type)) {
      const subject = event.anonymousVisitorId || event.sessionId || "anonymous";
