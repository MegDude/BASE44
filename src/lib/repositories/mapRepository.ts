...
type MapFeedParams = SharedMapFeedRequest & { search?: string; categories?: string[]; types?: string[] };
type SearchIntentParams = { query?: string; userLocation?: unknown };
type AgentIntent = {
  category?: string;
...
function getFallbackIntentFilters(query = "") {
  ...
  if (intent.intentMode === "perks") return { categories: [], types: ["perk"], intent };
...
function filterItems(items, params: MapFeedParams = {}) {
  ...
  if (query) results = results.filter((item) => getTextIndex(item).includes(query));
  if (district && district !== "downtown") results = results.filter((item) => String(item.district || "").toLowerCase() === district);
  if (categories.size > 0) results = results.filter((item) => categories.has(item.category));
  if (types.size > 0) results = results.filter((item) => types.has(item.type) || types.has(item.entity_type));
...
function mergeSharedFeedItems(remoteItems = [], fallbackItems = []) {
  ...
  for (const item of merged) {
    ...
    const key = String(normalized.entity_id || normalized.id || `${normalized.type}-${normalized.name || normalized.title || "entity"}`);
    const existing = deduped.get(key);
    deduped.set(key, existing ? { ...normalized, ...existing, metadata: { ...(normalized.metadata || {}), ...(existing.metadata || {}) } } : normalized);
  }
...
function scoreItemForIntent(item, query = "", intent: AgentIntent = {}, placeNames = new Set()) {
  ...
  if (intent.categories?.includes(category)) score += 20;
  if (intent.types?.includes(type) || (type === "property" && intent.types?.includes("building"))) score += 18;
  ...
  if (intent.intentMode === "perks" && (item?.perk || item?.perk_value || type === "perk")) score += 26;
  return score;
}

function boostByAgent(items = [], query = "", intent: AgentIntent = {}, agentPlaces = []) {
  const normalizedQuery = String(query || "").trim().toLowerCase();
  const placeNames = new Set((agentPlaces || []).map((place) => String(place?.name || "").trim().toLowerCase()).filter(Boolean));
  return [...items].sort((a, b) => scoreItemForIntent(b, normalizedQuery, intent, placeNames) - scoreItemForIntent(a, normalizedQuery, intent, placeNames));
}
...
export const mapRepository = {
  ...
  async searchWithIntent({ query, userLocation }: SearchIntentParams = {}) {
    ...
    const fallbackIntent: AgentIntent = { ...fallback.intent, categories: fallback.categories, types: fallback.types, ranking: fallbackRanking, suggestions: [] };

    let apiIntent: AgentIntent = {};
    let agentPlaces = [];
    let source: "api" | "base44" | "fallback" = "fallback";

    const apiResponse = await mapAgentApi.askMap(trimmedQuery, { userLocation });
    ...
    if (source !== "api") {
      try {
        const intentResponse = await base44Api.invoke("searchMapIntent", { query: trimmedQuery, context: userLocation });
    ...
    const items = await this.getMapFeed({ query: trimmedQuery, filters: { categories: intent.categories || [], types: intent.types || [] } });
    const rankedItems = boostByAgent(items, trimmedQuery, intent, agentPlaces);

    void mapAgentApi.logSearch(trimmedQuery, { source, intentMode: intent.intentMode || null, category: intent.category || null, categories: intent.categories || [], types: intent.types || [] });
