...
export function buildWorkspacePerformanceReport({ workspace, analytics }) {
  ...
  return {
    ...
    launchReadiness: { ready: totalActivity > 0, totalActivity },
...
export async function downloadPerformancePdf(report) {
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Recommended next step", 48, y + 12);
  doc.setFont("helvetica", "normal");
  doc.text(doc.splitTextToSize(`${report.primaryRecommendation.title} ${report.primaryRecommendation.rationale}`, 510), 48, y + 28);
  doc.setFontSize(8);
