...
for (const staleKey of ["entityId", "perkId", "eventId", "collectionId", "listingId", "campaignId"]) {
  assert.match(transition, new RegExp(`${staleKey}: \\\"\\\"`), `intent transition clears ${staleKey}`);
}
assert.match(
  transition,
  /routeId: options\.routeId \|\| options\.collection \|\| ""/,
  "ordinary intents clear routeId while explicit route intents retain their canonical route",
);
for (const stateClear of ["setClusterDrawer(null)", "setMapAnswer(null)", "setSelectedId(\"\")", "clearScopedMapResults()", "recentScopedQueryRef.current = \"\""] ) {
...
assert.match(
  source,
  /className="dp-search-intent-status(?: sr-only)?" aria-live="polite"/,
  "intent result status is announced",
);
