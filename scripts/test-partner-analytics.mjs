...
for (const view of ["overview", "audience", "research", "places", "campaigns", "activity", "sources", "geography", "reports"]) assert.match(component, new RegExp(`\\[\\"${view}\\"`), `missing view: ${view}`);
for (const parameter of ["workspace", "organizationId", "view"]) assert.match(component, new RegExp(`params\\.get\\(\\"${parameter}\\"\\)`), `missing URL parameter: ${parameter}`);
assert.match(component, /Results appear after activity is recorded/);
assert.match(component, /No estimated performance/);
assert.match(component, /getResearchCoverageSummary/);
