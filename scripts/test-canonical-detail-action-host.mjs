...
assert.match(panel, /createPortal\(actions,\s*(?:drawerActionHost|drawerHost)\)/, "canonical actions must render outside the scroll container");
...
assert.match(css, /> \.dp-canonical-detail-actions\s*\{[\s\S]*position:\s*relative !important/, "ported actions must participate in the drawer grid");
assert.doesNotMatch(css, /> \.dp-canonical-detail-actions\s*\{[\s\S]{0,180}position:\s*fixed !important/, "canonical actions must not rely on transformed fixed positioning");
