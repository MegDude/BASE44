...
const finalRailCss = readFileSync("src/styles/search-console-readable-rail-final.css", "utf8");
...
assert.doesNotMatch(mapSource, /insertMoreAfterIndex:\s*moreToggleInsertIndex/, "More must not be inserted inside the primary rail");
assert.match(mapSource, /options\.includeMoreToggle && options\.insertMoreAfterIndex == null \? renderMoreButton\(\) : null/, "More must append at the end of the primary rail");
...
assert.match(finalRailCss, /dp-search-more-toggle/, "More needs the same final readability contract as primary shortcuts");
assert.match(finalRailCss, /--dp-search-console-gold:\s*#c8a96a/i, "Search console gold must use the canonical token value");
assert.match(finalRailCss, /dp-search-intent-audience-tabs/, "Audience switcher must use the same contrast contract");
assert.ok(
...
assert.ok(
  ...
  "Final search rail readability layer must load last",
);
