...
const recoveryStyles = readFileSync("src/styles/dp-recovery-final.css", "utf8");
...
assert.match(
  main,
  /search-intent-glass-surface-final\.css"[\s\S]*resident-qr-modal-final\.css"/,
  "resident QR authority must load after the shared surface styles",
);
...
assert.match(styles, /:is\(\.dp-resident-qr-back, \.dp-resident-qr-close\) svg[\s\S]*width:\s*15px\s*!important[\s\S]*height:\s*15px\s*!important/);
...
assert.doesNotMatch(recoveryStyles, /\.dp-resident-qr-status[^{]*\{[^}]*border-radius:\s*999px/is);
assert.doesNotMatch(recoveryStyles, /\.dp-resident-qr-status\.is-ready[^{]*\{[^}]*background:/is);
