...
async function resolveAttribution(database, entityId, explicitListingId, explicitPerkId) {
  const attribution = { partner_id: null, partner_organization_id: null, listing_id: null, perk_id: /^[0-9a-f-]{36}$/i.test(String(explicitPerkId || '')) ? explicitPerkId : null };
  ...
  const { data, error } = /^[0-9a-f-]{36}$/i.test(String(explicitListingId || ''))
...
export default async function handler(req, res) {
  ...
  const payload = { source_type: normalizeSourceType(sourceType), action_type: normalizeActionType(type), value: Number.isFinite(Number(value)) ? Number(value) : 1, session_token: sessionId || null, user_email: profileId || null, district: typeof req.body?.district === 'string' ? req.body.district : null, entity_id: entityId || null, entity_type: entityType || null, partner_id: attribution.partner_id, partner_organization_id: attribution.partner_organization_id, listing_id: attribution.listing_id, perk_id: attribution.perk_id };
  ...
  return res.status(200).json({ ok: true, attribution: { organizationId: attribution.partner_organization_id, listingId: attribution.listing_id } });
}